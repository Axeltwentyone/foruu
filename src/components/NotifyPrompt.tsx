...
import { currentSubscription, isIOS, isStandalone, pushSupported, subscribeToPush } from '../lib/push'

type State = 'checking' | 'hidden' | 'ios-hint' | 'ask' | 'denied' | 'on' | 'asking' | 'error'
...
export default function NotifyPrompt() {
  ...
  useEffect(() => {
    let alive = true
    ;(async () => {
      if (!pushSupported()) return setState(isIOS() && !isStandalone() ? 'ios-hint' : 'hidden')
